...
class CertificateUsersController extends Controller
{
    ...
    {
        $rows = $this->ApplyFilter($request,
            CmsUser::select('cms_users.*')
                ->where(['cms_roles.name' => CmsUser::CERTIFICATE_MANAGER])
                ->leftJoin('cms_user_role', 'cms_users.id', '=', 'cms_user_role.cms_user_id')
                ->leftJoin('cms_roles', 'cms_roles.id', '=', 'cms_user_role.cms_role_id')
                ->leftJoin('cert_config', 'cms_users.id', '=', 'cert_config.user_id')
        )->get();

        return view('certificate-users.index', compact(['rows']));
    }
