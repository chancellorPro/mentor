...
class GroupEventController extends Controller
{
    ...
    {
        $rows = $this->ApplyFilter($request, GroupEvent::query()->orderBy('created_at', 'desc'))->get();

        $asset_ids = [];
        foreach ($rows->toArray() as $k => $item) {
            foreach ($item['assets_setup'] as $i => $v) {
                $asset_ids[] = $v['asset_id'];
                $rows[$k]->push(['embed_box' => [$i => $v]]);
            }
        }

        $assets = Asset::whereIn('id', $asset_ids)->get()->keyBy('id')->toArray();

        return view('group-event.index', compact(['rows', 'assets']));
    }
    ...
    public function update(Request $request)
    {
        foreach ($request->get('ge') as $event) {
            GroupEvent::updateOrCreate(['id' => $event['id']], [
                ...
                'assets_setup' => json_encode($event['assets_setup'])
            ]);
        }
