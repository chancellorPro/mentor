        ...
        <div class="row">
            @if (!empty($file))
                <div class="col-md-3 {{$type}}-thumbnail file-box thumbnail">
                        ...
                        @if ($type == 'image')
                            <img class="img-responsive" src="{{ env('CDN_URL') . '/' . $file }}"
                                 title="@formFieldLabel($name)">
            ...
            <a class="btn btn-success btn-download" href="{{ env('CDN_URL') . '/' . $file }}"><i class="fa fa-download"></i></a>
            @endif
        </div>
