        ...
        <div style="width: 100%; margin-bottom: 20px;">
            ...
            <table id="send-container" border="0" style="width: 100%">
                        ...
                        <div id="assets_list">
                            <table>
                                @foreach ($event['embed_box'] as $item)
                                    @php
                                        if(!isset($default_prize)) {
                                            $default_prize = $item['asset_id'];
                                        }
                                    @endphp
                                    <tr>
                                        <td>
                                            <div class="assets_block">
                                                <img data-prize="{{ $item['asset_id'] }}"
                                                     src="{{ env('CDN_URL') . '/' . $assets[$item['asset_id']]['preview_url'] }}"
                                                     style="height: 55px; {{ ($default_prize == $item['asset_id']) ?
                            ...
                            <input type="hidden" id="asset_id" name="loot_data[asset_id]"
                                   value="{{ old('loot_data[asset_id]') ?? $default_prize }}">
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td style="">
                        <textarea style="resize: none" name="loot_data[message]" data-val="message"
                                  class="form-control limited" data-maxlength="145" maxlength="145"
                                  rows="10">{{ old('loot_data[message]') ?? '' }}</textarea><br>
                    </td>
                </tr>
                ...
                <tr>
                    <th colspan="2">
                        ...
                        <input type="button"
                               class="redirect"
                               value="Return to Special Prizes"
                               data-route="{{ route('special-prizes.index') }}">
                    </th>
                </tr>
            </table>
        </div>
