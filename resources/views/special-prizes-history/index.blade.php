@section('pageTitle', 'Special prizes history')
@extends('layouts.pages.config', [
    'title' => 'Special prizes history',
])
    ...
    <div class="table-responsive">
        <table class="table table-hover" id="trophy-history-users">
            ...
            <tr>
                <th>
                    <div class="row form-group">
                        <input autocomplete="off" placeholder="Date to"
                               ...
                               value="{{ old("date_to") ?? $filter['date_to'] ?? '' }}" name="date_to"
                               id="filter_date_to">
                        <input autocomplete="off" placeholder="Date from"
                               class="filter form-control datepicker pull-right"
                               value="{{ old("date_from") ?? $filter['date_from'] ?? '' }}" name="date_from" id="filter_date_from">
                    </div>
                </th>
                <th>
                    @if($isAdmin)
                        @include('layouts.filter-col', ['filterType' => 'string', 'field' => 'event_name'])</th>
                    @endif
                ...
                @if($isAdmin)
                    <th>@include('layouts.filter-col', ['filterType' => 'select', 'field' => 'cms_user_id', 'filterCollection' => $cmsUserList])</th>
                @endif
            ...
            @foreach($rows as $id => $row)
                <tr>
                    ...
                    <td style="text-align: center">{{ $row->event ? $row->event->name : '' }}</td>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            init_filter_col("{{ route('special-prizes-history.index') }}");
        })
    </script>
@endpush
